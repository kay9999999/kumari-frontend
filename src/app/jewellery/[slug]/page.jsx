...
import Category_Banner from "../../../components/custom/Category_Banner";
...
const SubCategoryPage = () => {
  ...
  const bannerIndexMap = {
    ...
    "diamond-earrings": 23,
    "gold-earrings": 24,
    "earrings-for-kids": 25,
    "earrings-for-women": 26,
    "diamond-pendant-necklace": 37,
    "gold-pendant-necklace": 38,
    "pendant-necklace-for-women": 39,
    "pendant-necklace-for-kids": 40,
    "diamond-bracelet": 48,
    "gold-bracelet": 49,
    "bracelet-for-women": 50,
    "bracelet-for-kids": 51,
    "lightweight-jewellery": 52,
    "nose-pins": 53,
    "rose-gold-chains": 54,
    "gold-coins": 55,
    mens: 56,
  ...
  const bannerIndex = bannerIndexMap[slug];
  ...
  const productsEndpoint = qs.stringify(
    {
      ...
      filters: {
        sub_categories: {
          name: slug, // Ensure only products of the current sub-category are fetched
        },
  ...
  const generateBreadcrumbs = () => {
    ...
    const pathSegments = [
      ...
      { title: heading, href: `/jewellery/${slug}` },
    ];
    return pathSegments;
  };
            ...
            <div className="mt-8">
              ...
              {filtersLoading ? (
                ...
                filters
                  ?.filter((filter) => {
                    // Exact match check
                    return filter.sub_categories.some(
                      (cat) =>
                        slug === cat.name.toLowerCase().replace(/ /g, "-") // Match exact slug
                    );
                  })
                      ...
                      {expandedFilter === filter.id && (
                        <div className="flex flex-col gap-2 mt-2">
                          {filter.filter_values
                            ?.filter((valueFilter) => {
                              // Exact match check
                              return valueFilter.sub_categories.some(
                                (cat) =>
                                  slug ===
                                  cat.name.toLowerCase().replace(/ /g, "-") // Match exact slug
                              );
                            })
                            ?.map((value) => (
                              <label
